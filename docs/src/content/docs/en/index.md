---
title: igcp-aforro
description: TypeScript library and CLI for simulating Portuguese IGCP Aforro Séries B, C, D, E, and F Treasury Certificates.
template: splash
hero:
  tagline: Deterministic, decimal-safe Aforro Séries B, C, D, E, and F simulator. Drop-in for JS/TS apps; ships with a CLI and a static `rates.json` for everyone else.
  actions:
    - text: Quickstart
      link: /en/quickstart/
      icon: right-arrow
...
`igcp-aforro` reproduces the [IGCP Aforro](https://www.igcp.pt/) calculation end-to-end for **Séries B, C, D, E, and F**:
...
The library covers **Série B** (subscriptions from 1 Jul 1986 to 25 Jan 2008; perpetual; TBA-based base rate), **Série C** (closed series, 10-year maturity), **Série D** (subscriptions open from 1 Feb 2015 to 31 Oct 2017, 10-year maturity), **Série E** (subscriptions open from 1 Nov 2017 to 1 Jun 2023, 10-year maturity), and **Série F** (subscriptions open from 1 Jun 2023 onwards, 15-year maturity) end-to-end. **Série A** (legacy) remains out of scope. The generated [API reference](/api/) is English-only for now because it is built from the package’s TSDoc comments.
